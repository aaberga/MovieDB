//
//  MovieDB_MoviesCollection_API.swift
//  MovieDB
//
...
private let kAPIURLString = "https://api.themoviedb.org/3/collection/"
...
class MovieDB_MoviesCollection_API {
    
    static func sendRequest(withID collectionID: Int, andOnCompletion completionBlock: @escaping (_ result: Any?, _ error: Error?) -> Void) {
        ...
        /* Create the Request:
         Get Movies In Collection (GET https://api.themoviedb.org/3/collection/10)
         */
        
        let commandURL = kAPIURLString + "%ld"
        guard var URL = URL(string: String(format: commandURL, collectionID)) else {return}
